...
\begin{alineas}
    ...
    \item Cabo de rede Gigabit Ethernet;
...
\begin{quadro}[htb]
    ...
    \begin{tabularx}{\linewidth}{|c|>{\raggedright\arraybackslash}X|}
        ...
        \hline
        \hline
        1 & Gravação da imagem Armbian minimal no cartão microSD com Raspberry Pi Imager \\
        \hline
        2 & Inicialização da ROCK 4B+ e configuração de rede (IP fixo) \\
        \hline
        3 & Atualização do sistema com \texttt{apt update \&\& apt upgrade} \\
        \hline
        4 & Instalação do OpenMediaVault via script oficial \\
        \hline
        5 & Configuração dos discos: formatação ext4, montagem, pastas compartilhadas \\
        \hline
        6 & Instalação do Docker e Docker Compose \\
        \hline
        7 & Implantação do Nextcloud via Compose (PHP, MariaDB, Redis) \\
        \hline
        8 & Configuração do Tailscale via Compose ou pacote oficial \\
        \hline
        9 & Criação de usuários e permissões no OMV e Nextcloud \\
        \hline
...
Os testes serão conduzidos em dois cenários: rede local (Gigabit Ethernet cabeada) e acesso remoto (via Tailscale). Cada teste será executado três vezes para garantir reprodutibilidade, e o valor médio será registrado.

\subsection{Teste de Throughput Local}

O throughput de leitura e escrita será medido com as seguintes ferramentas e comandos:
...
\begin{itemize}
    ...
    \item \textbf{iperf3}: Medição de throughput de rede (\texttt{iperf3 -c <ip> -t 30});
    \item \textbf{rsync}: Transferência de arquivos grandes (1 GB) entre máquina cliente e NAS;
    \item \textbf{hdparm}: Leitura de cache e buffer do disco (\texttt{hdparm -Tt /dev/sda}).
\end{itemize}

\subsection{Teste de Throughput Remoto}

O acesso remoto será testado através do Tailscale, com os mesmos procedimentos do teste local, porém utilizando o endereço IP atribuído pela malha VPN. A latência será medida com \texttt{ping} e \texttt{mtr}.
...
\begin{quadro}[htb]
    ...
    \begin{tabularx}{\linewidth}{|c|>{\raggedright\arraybackslash}X|c|c|}
        ...
        \hline
        C5 & Upload Nextcloud (100 MB) & Local & HTTPS \\
        \hline
        C6 & Download Nextcloud (100 MB) & Local & HTTPS \\
        \hline
        C7 & Leitura sequencial remota & Tailscale & SMB \\
        \hline
        C8 & Escrita sequencial remota & Tailscale & SMB \\
        \hline
        C9 & Upload Nextcloud remoto & Tailscale + HTTPS & HTTPS \\
        \hline
...
\begin{quadro}[htb]
    ...
    \begin{tabularx}{\linewidth}{|>{\raggedright\arraybackslash}X|>{\raggedright\arraybackslash}X|}
        ...
        \hline
        \hline
        \texttt{dd} & Medição de throughput de leitura/escrita sequencial em disco \\
        \hline
        \texttt{hdparm} & Medição de velocidade de cache e buffer do disco \\
        \hline
        \texttt{iperf3} & Medição de throughput de rede TCP/UDP \\
        \hline
